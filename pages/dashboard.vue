...
const todoTasks = computed(() => tasks.value.filter(task => task.status === 'todo').length)
...
const overdueTasks = computed(() => tasks.value.filter(task => task.status !== 'done' && Date.parse(task.dueDateTime) < Date.now()).length)
...
<template>
  <main class="mx-auto max-w-5xl min-h-screen p-8">
    <header class="mb-8">
      ...
      <p class="text-gray-600">At-a-glance summary of caseworker workload.</p>
    </header>
    ...
    <div class="mt-6">
      <NuxtLink to="/tasks" class="rounded bg-gray-900 px-4 py-2 text-white">Go to tasks page</NuxtLink>
    </div>
  </main>
</template>
